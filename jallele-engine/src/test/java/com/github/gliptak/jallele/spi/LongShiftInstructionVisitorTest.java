...
public class LongShiftInstructionVisitorTest {
	...
	public final void testIsMatch() {
		...
		int opCode=Opcodes.LSHL;
		vs.setOpCode(opCode);
		Random random = new Random();
		LongShiftInstructionVisitor v=new LongShiftInstructionVisitor(random);
		VisitStatus vsNew=v.isMatch(vs);
		assertThat(vsNew.getOpCode(), IsNot.not(vs.getOpCode()));
		assertThat(vsNew.getOpCode(),
				anyOf(equalTo(Opcodes.LSHL), equalTo(Opcodes.LSHR), equalTo(Opcodes.LUSHR)));
